...
class vgg16:
    ...
    def load_weights(self, weight_file, sess):
        weights = np.load(weight_file)  # 权重载入

        keys = sorted(weights.keys())  # 对获得的字典值进行排序
        for i, k in enumerate(keys):  # 之后使用一个enumerate方法将数据迭代出，这里迭代的结果是序号以及key值，
            print(i, k, np.shape(weights[k]))
            sess.run(self.parameters[i].assign(weights[k]))  # 之后执行一个赋值操作将对应的权重值赋值到参数列表中
